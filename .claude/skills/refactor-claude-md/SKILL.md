...
path-scoped rules. In hqgit, `CLAUDE.md`, `.claude/rules/`, and
...
- Per-crate implementation notes once a crate has shipped (for example
  the REAPI digest mapping for `crates/hqgit-eval/**`).
- Testing patterns (fixture ledgers, the `testing` feature builders).
- The Go executor's build and sandbox conventions (`executor/**`).
