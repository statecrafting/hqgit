---
name: cleanup
description: Run dead-code and duplicate-code detection across the hqgit crates, investigate each finding in context, and return categorized recommendations that respect spec ownership
...
detection across `crates/`, `fuzz/`, `executor/`, and `web/`, reads each
...
You are a cleanup analyzer for hqgit. Analyze and report; change nothing.
...
Go (`executor/`): `go vet ./...` and `staticcheck ./...` if installed.
...
library crates consumed by `hqgit-cli` or `hqgit-server`; test fixtures
and `testing` feature builders; fuzz targets; workflow and hook scripts;
...
`.derived/`); never recommend removing a golden vector, a fuzz target, or
