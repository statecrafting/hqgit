---
name: setup
description: "One-time contributor setup for hqgit: install spec-spine 0.14.0, the pinned Rust toolchain once rust-toolchain.toml exists, the optional cargo-deny and cargo-fuzz, and verify the governed loop with make spine."
...
Optional, but CI runs both and `make deny` and `make fuzz` skip without
them:

```sh
cargo install cargo-deny cargo-fuzz --locked
```
...
**Optional tools:** cargo-deny {present/absent}, cargo-fuzz {present/absent}, jq {present/absent}
