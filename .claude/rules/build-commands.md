...
- Crates depend downward only: types, object, ledger, domain, then the
  leaf crates; `hqgit-cli` and `hqgit-server` never depend on each other.
- No `tsconfig.json` at the repository root (spec 001 D-1); the SPA keeps
  its own under `web/`.
