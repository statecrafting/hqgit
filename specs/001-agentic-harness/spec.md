...
0.17.0 in every site that states it (`govern.yml`, `AGENTS.md`, `README.md`,
the architect agent). As in D-4 the corpus was verified byte-compatible
first: 0.17.0's `compile --check` and `index check` both report fresh
against shards written by 0.14.0, and no registry shard changes in this
commit. Unlike D-4, two gate steps refuse under the new pin, and both
refusals are correct.

`lint --fail-on-warn` reported seven `L-008` warnings (spec-spine 057):
claimed paths that no content hash covers, so their contents could change
without staling any shard. The cause was in this repository's own config.
Six of the ten `[index] extra_hashed_inputs` patterns ended in `**`, which
matches directories only and contributes no bytes, so `standards/`,
`.github/workflows/`, `.claude/agents/`, `.claude/rules/`, `.claude/skills/`
and `docs/design/` were folded into the global scalar by nothing at all.
Every one is rewritten as `**/*`, and `scripts/**/*`, `.mcp.json` and
`.github/dependabot.yml` are added for the four claims that had no covering
pattern. `lint` is now clean. Because those files finally contribute bytes,
the global scalar moves and all 68 index shards restale; they are
regenerated and committed here. This was a live governance hole, not a
lint preference: an edit to `AGENTS.md` or a rule under `.claude/` staled
...
`extends` crossing that cleared a change (052), the `L-008` lint that found
the dead globs above (057), `registry plan` answering blocked as well as
